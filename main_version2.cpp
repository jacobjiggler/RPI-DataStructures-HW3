...
void MoreTests() {
  ...
  //
	...
	JaggedArray<int> janumber(7);

  assert (janumber.numBins() == 7);
  assert (janumber.numElements() == 0);
  assert (janumber.isPacked() == false);

  // intialize the values in the JaggedArray
  janumber.addElement(1,1);
  janumber.addElement(1,2);
  janumber.addElement(1,3);
  janumber.addElement(3,4);
  janumber.addElement(3,5);
  janumber.addElement(6,6);

  // verify data is correct
  std::cout << "STARTS UNPACKED" << std::endl;
  janumber.print();
  assert (janumber.numBins() == 7);
  assert (janumber.numElements() == 6);
  assert (janumber.numElementsInBin(0) == 0);
  assert (janumber.numElementsInBin(1) == 3);
  assert (janumber.numElementsInBin(2) == 0);
  assert (janumber.numElementsInBin(3) == 2);
  assert (janumber.numElementsInBin(4) == 0);
  assert (janumber.numElementsInBin(5) == 0);
  assert (janumber.numElementsInBin(6) == 1);
  assert (janumber.getElement(1,0) == 1);
  assert (janumber.getElement(1,1) == 2);
  assert (janumber.getElement(1,2) == 3);
  assert (janumber.getElement(3,0) == 4);
  assert (janumber.getElement(3,1) == 5);
  assert (janumber.getElement(6,0) == 6);
  ...
  // add '7' and verify
  janumber.addElement(3,'7');
  std::cout << "ADDED 7" << std::endl;
  janumber.print();
  assert (janumber.numBins() == 7);
  assert (janumber.numElements() == 7);
  assert (janumber.numElementsInBin(0) == 0);
  assert (janumber.numElementsInBin(1) == 3);
  assert (janumber.numElementsInBin(2) == 0);
  assert (janumber.numElementsInBin(3) == 3);
  assert (janumber.numElementsInBin(4) == 0);
  assert (janumber.numElementsInBin(5) == 0);
  assert (janumber.numElementsInBin(6) == 1);
  assert (janumber.getElement(1,0) == 1);
  ...
  // remove '7' and verify
  janumber.removeElement(3,2);
  std::cout << "REMOVED 7" << std::endl;
  janumber.print();
  assert (janumber.numBins() == 7);
  assert (janumber.numElements() == 6);
  assert (janumber.numElementsInBin(0) == 0);
  ...
  assert (janumber.getElement(3,1) == 5);
  assert (janumber.getElement(6,0) == 6);
// copy janumber to a new jagged array and test it before clearing it 
// JaggedArray<int> copya(janumber);
//copya.print();
  ...
  assert (janumber.numElements() == 0);
  assert (janumber.numElementsInBin(0) == 0);
  assert (janumber.numElementsInBin(1) == 0);
  assert (janumber.numElementsInBin(2) == 0);
  assert (janumber.numElementsInBin(3) == 0);
  assert (janumber.numElementsInBin(4) == 0);
  assert (janumber.numElementsInBin(5) == 0);
  assert (janumber.numElementsInBin(6) == 0);
...
void BatchTest(const char* filename, int iters) {
  ...
  while (iters > 0) {
    ...
    JaggedArray<char> *jab = new JaggedArray<char>(b);
    ...
    while (istr >> token) {
      if (token == "addElement") {
        ...
        istr >> b >> i >> c;
        char c2 = jab->getElement(b,i);
        assert (c == c2);
      } else if (token == "removeElement") {
        istr >> b >> i >> c;
	char c2 = jab->getElement(b,i);
        assert (c == c2);
	jab->removeElement(b,i);
      } else if (token == "pack") {
        jab->pack();
    ...
    // print out the jabgged array contents at the end of the last iteration
    if (iters == 0) {
      if (jab->isPacked()) {
	jab->unpack();
      }
